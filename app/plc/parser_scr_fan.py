...
class SCRFanParser:
    ...
    def __init__(self, 
        ...
        """
        # 使用绝对路径，避免工作目录问题
        self.db_config_path = Path(db_config_path) if db_config_path else self.PROJECT_ROOT / "configs" / "config_scr_fan_10.yaml"
    ...
    def load_config(self):
        ...
        logger.info("[SCRFanParser] DB%s解析器初始化完成: %s个设备, DB%s, 总大小%s字节",
                     self.db_config['db_number'], len(self.devices),
    ...
    def parse_module(self, module_config: Dict, data: bytes, offset: int) -> Dict[str, Any]:
        ...
        for field in base_module['fields']:
            ...
            try:
                ...
                logger.warning("[SCRFanParser] 解析字段失败 %s.%s @ offset %s: %s", module_type, field_name, offset + field_offset, e)
                parsed_fields[field_name] = {
                    'value': 0.0,
                    'display_name': field.get('display_name', field_name),
                    'unit': field.get('unit', '')
    ...
    def parse_all(self, db8_data: bytes) -> List[Dict[str, Any]]:
        ...
        for device in self.devices:
            try:
                ...
                logger.warning("[SCRFanParser] 解析设备失败 %s: %s", device['device_id'], e)
